...
5. **絶対 μA/V² 較正（F5 では相対単位を一次成果として deferred、recipe を記録）**:
   絶対前因子は **Hughes-Sipe 1996（PRB 53, 10751, in-repo）Appendix B Eqs.(B6)/(B7)** に明示:
   shift current 部は `σ ~ (e³/ℏ²) ∫dk/(4π³) Im[...]`（`4π³ = (2π)³/2` が BZ 測度）。Young & Rappe 2012 Eq.(1)
   の `πe³/(ℏ⁴) ∫d³k/(2π)³` と整合（規約差で ℏ 冪・係数 2 が動く）。本実装の `(1/N_k)Σ_k`（= `(V_cell/(2π)³)∫d³k`
   の BZ 平均）に `e³/(ℏ² V_cell)` 級の係数 + 周波数↔エネルギー換算を掛けて A/V² 化する。
   **ハルシネーション回避方針**: SI 換算は規約敏感なので、係数を独自導出に依存せず **Tan & Rappe 2015
   (arXiv:1508.03564, in-repo) の CsPbI₃ 公表 σ 値にアンカー**して較正する（次回の慎重なパスで実施）。
   Blount 1962 限界で絶対値は系統的過小（§6）。
